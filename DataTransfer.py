...
class MyHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        ...
        while not self.stop_socket:
            self.server_runtimes += 1
            self.to_client.send(str(1))
            self.to_client.recv(8)
            file = str(event.src_path).split('\\')[-1]
            # server move ; client move
            if not self.stop_socket:
                if file[-4:] == ".csv":
                    file_size = os.path.getsize(dataPath + '/' + file)
                    # print('正在传输文件：%s' % file)
                    print('正在传输文件：' + file + '。')
                    file_info = '%s|%s' % (file, file_size)
                    # print(type(file_info))
                    self.to_client.send(file_info)
                    self.to_client.recv(6)
                    fout = open(dataPath + '/' + file, "rb")
                    has_send = 0
                    # to_client.send(d)
                    while has_send < file_size:
                        frame_data = fout.read(1)
                        self.to_client.send(frame_data)
                        has_send += len(frame_data)
                    fout.close()
                    T.sleep(2)
                    if not os.path.isdir(dataPath + '/server-move'):
                        os.makedirs(dataPath + '/server-move')
                    try:
                        shutil.move(dataPath + '/' + file, dataPath + '/server-move/' + '/')
                    except:
                        messagebox.showwarning('转移失败', '文件转移失败，请手动转移%s文件，以免重复上传' % file)
                else:
                    print ("非csv文件不传输")
            ...
            if self.stop_socket:
                ...
                T.sleep(10)
                print("server run %s times" % self.server_runtimes)
                print("暂无数据传输，服务器已经监测文件夹%s次。" % str(self.server_runtimes))
        print("stoped by stop_socket variable")
    ...
    def StartSocketServer(self):
        ...
        port = 8105
        addr = (host, port)
        self.s_server.bind(addr)  # 绑定端口
        self.s_server.listen(5)  # 开始监听TCP传入连接，backlog指定在拒绝链接前，操作系统可以挂起的最大连接数，该值最少为1，大部分应用程序设为5就够用了
        self.server_runtimes = 0
        self.file_num = 0
        self.to_client, self.addr = self.s_server.accept()
        # 注意：accept()函数会返回一个元组
        # 元素1为客户端的socket对象，元素2为客户端的地址(ip地址，端口号)
        print ('...connected from :', addr)
        rev_str = self.to_client.recv(1024)
    # end of server module
